    ...
    tr:nth-child(even) {background-color: #f2f2f2}
      ...
      <body data-home-page="View-Tickets.html" data-home-page-title="View Tickets" class="u-body"><header class="u-clearfix u-header u-header" id="sec-fe07"><div class="u-clearfix u-sheet u-sheet-1">
          ...
          <a href="addbus.php">Click here to add Bus.</a>
          <table>
    <tr>
    <th>Bus Type</th>
    <th>Bus Name</th>
    ...
    $sql = "SELECT type, busname, arrivalcity, destinationcity, arrivaltime, destinationtime, fare FROM Buses";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["type"]. "</td><td>" . $row["busname"]. "</td><td>" . $row["arrivalcity"] . "</td><td>" . $row["destinationcity"] . "</td><td>"
    . $row["arrivaltime"]. "</td><td>" . $row["destinationtime"]. "</td><td>" . $row["fare"] . "</td></tr>";
    }
    echo "</table>";
    } else { echo "0 results"; }
    $conn->close();
